...
export default function SearchInput({ text, userInput, setUserInput, setIsSearchButtonPushed, type }) {
    ...
    return (
        <div className={styles.search_input}>
            <button className={styles.search_input__button} onClick={() => { userInput ? navigate(`search?q=${userInput}&type=${type}`) : null; setIsSearchButtonPushed(true) }} type='button' aria-label='Search'>
            ...
            <input
                ...
                onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                        ...
                        e.target.value ? navigate(`search?q=${e.target.value}&type=${type}`) : null;
